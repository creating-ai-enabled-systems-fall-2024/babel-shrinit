...
def filter(bboxes, class_ids, scores, nms_iou_threshold):
    """
    Filters bounding boxes using the Non-Maximum Suppression (NMS) algorithm.

    Parameters:
    - bboxes (list of tuples): Bounding boxes in the format [(x, y, width, height), ...].
    - class_ids (list of ints): Class IDs corresponding to each bounding box.
    - scores (list of floats): Confidence scores for each bounding box.
