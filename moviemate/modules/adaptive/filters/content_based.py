...
class ContentBasedFiltering:
    ...
    def recommend(self, user_id, top_n=10):
        ...
        -------
        pd.DataFrame
            A DataFrame with item IDs and similarity scores.
        """
        user_profile = self._get_user_profile(user_id)
        scores = {}
        ...
        return pd.DataFrame(top_items, columns=["item", "score"])
    ...
    def precision_at_k(self, user_id, k=10):
        ...
        float
            Precision@K value.
        """
        recommended = self.recommend(user_id, top_n=k)['item'].tolist()
        user_ratings = self.ratings[self.ratings['user'] == user_id]
        relevant_items = user_ratings[user_ratings['rating'] >= 4]['item'].tolist()  
...
if __name__ == "__main__":
    ...
    precision = recommender.precision_at_k(user_id, k=10)
